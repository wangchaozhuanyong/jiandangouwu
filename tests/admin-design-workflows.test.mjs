...
test("安全事件页面只投影真实审计信号且不伪造威胁检测结论", () => {
  ...
  const contracts = read("packages/contracts/src/audit.ts");
  const preview = readOptional("apps/admin/src/pages/design-preview-page.tsx");
  const css = read("apps/admin/src/styles.css");

  assert.match(app, /lazy\(\(\) => import\("\.\/features\/security-events\/security-events-page"\)\)/u);
  assert.match(app, /page === "security-events"[\s\S]*?<SecurityEventsPage/u);
  assert.match(page, /useCachedAdminResource<AuditEventPage>/u);
  assert.match(page, /getAuditPage\(query,\s*signal\)/u);
  assert.match(page, /服务器已在完整审计历史中筛选安全信号/u);
  assert.match(page, /security-event-pagination/u);
  assert.match(page, /应用筛选/u);
  assert.doesNotMatch(page, /最近 100 条审计记录|filterSecurityEvents/u);
  assert.doesNotMatch(page, /安全评分|实时风险事件|已阻止行为/u);
  assert.match(model, /classifySecurityAuditEvent/u);
  assert.match(model, /readSecurityEventQuery/u);
  assert.match(model, /securityEventQueryFromFilter/u);
  assert.match(contracts, /"auth\.sites\.bootstrap"/u);
  assert.match(contracts, /"settings\.storefront\.updated"/u);
  assert.match(contracts, /"support\.channel\.updated"/u);
  assert.match(contracts, /"notifications\.telegram\.intent\.updated"/u);
