...
- `apps/sites`：当前 Sites 生产运行面，使用 Vinext/Next、D1、R2 与 ChatGPT 登录。公开商品、人工订单和管理后台共用 D1；R2 保存媒体对象以及 AES-256-GCM 加密的 D1 快照。`CLOUDBRIDGE_DATA_KEY` 只作为根密钥，新联系方式、快照、恢复令牌和恢复证明通过 HKDF-SHA-256 派生独立 v2 子密钥。
...
- 备份下载始终是密文。新快照使用用途隔离的 v2 派生密钥和认证上下文，旧 v1 快照继续使用根密钥兼容读取；后台“重新校验”证明 R2 对象可读取、校验和一致、对应版本密钥可解密且逐表记录数一致。“验证恢复包”进一步检查主键、外键式关联、JSON 文档和 v1/v2 订单联系方式密文，但不会向当前 D1 写入任何业务记录。
...
- 根级 Next 版本与 `apps/storefront`、`apps/sites` 保持同一精确值，作为 npm workspace 的统一依赖解析锚点，使根级安全覆盖同时约束两个应用；它不创建第三个 Next 运行时。Next 内部 PostCSS 与 Sharp 的覆盖版本必须由锁文件、`npm ls`、生产构建和 `npm run audit:prod` 一同验证。
- `infra` 的 AWS CDK 库只用于类型检查、构建和 `synth`，归类为开发依赖，不进入网站运行依赖审计或运行容器；完整开发依赖审计中的 CDK 内置公告仍须在 AWS staging 前处理或批准，不能因为生产审计为 0 而忽略。
...
- 成员与角色权限集中在 `apps/api/src/access` 和 `apps/admin/src/features/access`：复用现有 RBAC 关系表，不引入第二套角色数据；成员列表使用显式安全投影，只返回认证提供方、密码是否已配置、TOTP 布尔值、失败次数和锁定截止时间，不返回密码派生、TOTP 密钥、Token 或会话标识。
- access 写入使用最近认证、业务原因、客户端确认、`updatedAt` CAS、Serializable 事务和同事务审计；自改角色/账号生命周期、最后一名正常超级管理员降权或停用，以及 `SUPER_ADMIN` 资料、权限与生命周期变更在服务端失败关闭。非系统角色支持创建、双语资料编辑、权限编辑与空角色删除，角色键保持不可变，仍有关联成员的角色不能删除。账号启停、解锁与其他成员 TOTP 重置还会通过 `SessionService` 撤销目标成员全部 Valkey 会话和 TOTP 登录/绑定流程。
- 后台会话仍存于 Valkey，但守卫在每次受保护请求从 MySQL 重新派生账号状态与权限，并在变化时同步会话快照；权限缓存不能成为撤销授权后的继续访问依据。
- MySQL 安全中心通过 `SCAN admin-session:*` 分批读取 Valkey 会话并在服务端按当前账号过滤，禁止使用阻塞式 `KEYS`。会话响应只投影随机会话 ID、当前标记和三个时间，不采集设备、IP 或浏览器指纹；单个和批量撤销都限制为当前账号、保护当前会话并写 MySQL 审计。Sites 身份仍由 ChatGPT 管理，不复制这套会话存储。
...
- 工作台真实性模型集中在 `apps/admin/src/features/dashboard`：从 `Overview` 响应派生未启用商品、最近订单样本与最近订单时间，并直接展示服务端全库库存风险摘要。MySQL 使用同一数据库事务读取在售总数、三类计数和各类样本；Sites 使用参数绑定 D1 查询。固定规则把库存数据冲突置于售罄和 1–3 件低库存之前，最多返回六项。
- 库存风险状态为 `IMPLEMENTED_LIVE_QUERY`，只证明本次数据库查询，不创建告警表、库存流水、通知或历史。工作台其他入口继续导航到订单、商品、通知和安全事件页面；目标页权限与证据边界独立，导航成功不能反向证明检测或投递服务已经运行。
...
- 审计读取支持完整匹配历史的服务端分页、关键词、结果、来源、目标类型和时间范围筛选；同一筛选可通过受保护的 POST 生成最多 5,000 条 CSV。共享契约负责固定列序、引号与公式注入防护，MySQL 与 Sites 服务端各自负责权限、身份、CSRF、原因、确认、计数上限和审计，不允许前端先遍历分页再拼接未受控下载。
- 商品管理读取支持完整目录的服务端分页、字面关键词和状态筛选。审计、安全事件与商品查询都要求 MySQL 计数与列表复用同一条件，并由 Sites 返回相同语义。数据安全页面仍只读取第一页最多 100 条作为近期投影；差异授权、不可变归档和保留策略需要单独设计。
- 安全事件工作台集中在 `apps/admin/src/features/security-events`：复用受 `audit.read` 保护的现有审计 API，由 `packages/contracts` 统一 MySQL 与 Sites 动作别名、分类和复核级别，服务端在完整审计历史中先筛选再分页并返回完整安全摘要；前端只投影安全白名单字段，不新增安全事件表、检测服务、写接口或权限码。
