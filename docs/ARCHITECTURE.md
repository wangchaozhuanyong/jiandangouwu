...
- 备份下载始终是密文。后台“重新校验”只证明 R2 对象可读取、校验和一致、密钥可解密且逐表记录数一致，不代表已执行生产恢复；覆盖恢复仍要求隔离 D1、维护窗口和回滚批准。
...
- 审计日志筛选模型集中在 `apps/admin/src/features/audit`，页面入口只组合分页元数据、筛选、单行表与安全详情。通用 `AdminService.auditEvents` 使用 Prisma 显式字段投影，禁止依赖前端 TypeScript 类型隐藏数据库中的 `beforeData`、`afterData`、`ipHash` 或内部操作者 ID。
- 当前审计读取仍固定为最近 100 条并返回数据库总数；关键词、结果、来源、目标类型和时间筛选在已加载窗口内执行。完整历史检索、服务器筛选、分页、差异授权、导出和保留策略需要单独设计，不能由前端补写为已实现。
