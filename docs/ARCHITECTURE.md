...
- access 写入使用最近认证、业务原因、客户端确认、`updatedAt` CAS、Serializable 事务和同事务审计；自改角色/账号生命周期、最后一名正常超级管理员降权或停用，以及 `SUPER_ADMIN` 权限变更在服务端失败关闭。账号启停、解锁与其他成员 TOTP 重置还会通过 `SessionService` 撤销目标成员全部 Valkey 会话和 TOTP 登录/绑定流程。
