...
- Sites 备份：使用真实 SQLite migration 与内存 R2 验证每日唯一快照、AES-GCM 密文、R2 回读、SHA-256、解密、逐表记录数、重新校验和禁止缓存下载。恢复测试必须同时覆盖用途隔离的 v2 快照/联系方式/令牌/证明、旧 v1 快照与联系方式兼容读取、主键/关联/JSON/加密联系方式逻辑验证、一次性 RSA-OAEP 转移包、内存 SQLite 全表导入与回读、`foreign_key_check`、限时令牌和 HMAC 完成证明；可选新 D1 候选必须核对 `0700/0600`、拒绝覆盖、SQL/验证文件 SHA-256、逐表清单，并把生成的 `restore.sql` 重新导入空 SQLite 验证。损坏关联、篡改记录数、错误证明或过期转移必须失败关闭，测试输出不得显示密钥或明文联系方式。五项异常门禁中，外部告警未真实送达时必须保持失败。出现 v2 生产写入后，不得回滚到缺少 v2 读取层的旧 Worker；必须保持兼容版本或使用前向修复。
