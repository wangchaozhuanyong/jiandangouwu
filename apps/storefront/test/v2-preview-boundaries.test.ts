...
const combinedSource = (files: SourceFile[]) =>
  files
...
test("storefront V2 preview never imports order or admin mutations", () => {
  ...
  assert.match(
    source,
    /import\s*\{\s*getConfig\s*\}\s*from\s*["'][^"']*\/lib\/api["']/u,
  );
  for (const disallowedRead of [
    "getCategories",
    "getProducts",
    "getProduct",
...
test("storefront preview fixtures are unmistakably DEMO data with bilingual truth copy", () => {
  ...
  const ids = [...fixtures.matchAll(/\bid\s*:\s*["'`]([^"'`]+)["'`]/gu)].map(
    (match) => match[1] ?? "",
  );
...
test("storefront preview exposes the approved channel and order-lookup pages", () => {
  ...
  for (const label of [
    ...
    assert.ok(
      ...
      `missing bilingual preview navigation label: ${label}`,
    );
  }
...
test("site navigation, product categories, and the DEMO cart stay separate", () => {
  ...
  const styles = readFileSync(
    join(storefrontRoot, "app", "globals.css"),
    "utf8",
  );
  ...
  assert.match(
    pages,
    /className=\{`v2-preview-catalog__main\$\{catalogLocked \? " is-scroll-locked" : ""\}`\}/u,
  );
  assert.match(pages, /controlsTop <= 68\.5/u);
  assert.match(
    pages,
    /className="v2-preview-scenario"[\s\S]*?<details>[\s\S]*?<summary>/u,
  );
  ...
  assert.doesNotMatch(
    shell,
    /<small>CloudBridge V2<\/small>|>\{locale === "zh" \? "浏览频道" : "Explore channels"\}</u,
  );
  assert.doesNotMatch(shell, /0\{index \+ 1\}/u);
  assert.match(
    shell,
    /const showTransitFloat = pathname === base \|\| pathname === `\$\{base\}\/`/u,
  );
  assert.match(
    shell,
    /<span>\{locale === "zh" \? "中转站" : "Transit"\}<\/span>/u,
  );
  assert.match(shell, /!isProductDetail && !isCartPage && !isHome && footer/u);

  assert.match(
    styles,
    /\.v2-preview-footer \{[^}]*grid-template-rows:\s*128px 52px;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-footer__brand > span \{[^}]*width:\s*48px;[^}]*height:\s*34px;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-product-grid\s*\{[^}]*grid-template-columns:\s*repeat\(5,\s*minmax\(0,\s*1fr\)\)/u,
  );
  assert.match(styles, /\.v2-preview-cart-layer \{[^}]*position:\s*fixed;/u);
  assert.match(
    styles,
    /\.v2-preview-mobile-bottom-nav \{[^}]*display:\s*none;/u,
  );
  assert.match(
    styles,
  ...
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-secondary-categories \{[^}]*width:\s*76px;[^}]*height:\s*max-content;[^}]*overflow-y:\s*hidden;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 330px\)[\s\S]*?\.v2-preview-catalog__main\s*\{[^}]*grid-template-columns:\s*72px minmax\(0,\s*1fr\)/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-product-card\s*\{[^}]*height:\s*104px;[^}]*grid-template-columns:\s*clamp\(84px,\s*24vw,\s*92px\)/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-product-card__image \{[^}]*aspect-ratio:\s*1\s*\/\s*1;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-product-card__image > img \{[^}]*object-fit:\s*contain;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-product-card__body \{[^}]*height:\s*92px;[^}]*grid-template-rows:\s*28px 20px 44px;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-mobile-bottom-nav\s*\{[^}]*grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\)/u,
  );
  assert.match(
    styles,
    /\.v2-preview-transit-float \{[^}]*width:\s*56px;[^}]*min-width:\s*56px;[^}]*height:\s*56px;[^}]*border-radius:\s*50%;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-transit-float::before \{[^}]*border-radius:\s*inherit;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-transit-float > span \{[^}]*display:\s*block;[^}]*white-space:\s*nowrap;/u,
  );
  assert.doesNotMatch(
    styles,
    /\.v2-preview-transit-float \{[^}]*min-width:\s*(?:108|124)px;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-brand > span \{[^}]*overflow:\s*visible;[^}]*border:\s*0;[^}]*background:\s*transparent;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-product-card__hit \{[^}]*position:\s*absolute;[^}]*inset:\s*0;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-detail-visual \{[^}]*aspect-ratio:\s*1\s*\/\s*1;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-header__actions > \.v2-preview-cart-button \{[^}]*display:\s*none;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-header__actions > \.v2-preview-support-button \{[^}]*display:\s*inline-flex;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-header__actions > \.v2-preview-icon-button,[\s\S]*?\.v2-preview-header__actions > \.language-picker,[\s\S]*?\.v2-preview-header__actions > \.v2-preview-support-button \{[^}]*width:\s*44px;[^}]*height:\s*44px;[^}]*border-radius:\s*12px;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-catalog__toolbar\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\) minmax\(104px,\s*116px\);[^}]*grid-template-rows:\s*50px;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-catalog__search \{[^}]*grid-column:\s*1;[^}]*grid-row:\s*1;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-catalog__toolbar \.currency-picker \{[^}]*grid-column:\s*2;[^}]*grid-row:\s*1;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-header \{[^}]*position:\s*sticky;[^}]*z-index:\s*230;[^}]*top:\s*0;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-catalog__main \{[^}]*height:\s*calc\(100dvh - 134px - env\(safe-area-inset-bottom\)\);/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-catalog__controls \{[^}]*position:\s*sticky;[^}]*top:\s*68px;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-catalog__main\.is-scroll-locked > \.v2-preview-secondary-categories,[\s\S]*?\.v2-preview-catalog__results\s*\{[^}]*overflow-y:\s*auto;/u,
  );
});
...
test("home removes the rejected content blocks while other pages retain the editorial footer", () => {
  ...
  assert.doesNotMatch(
    homeSource,
    /PreviewChannelCards|PreviewAssurance|PreviewProcessAndFaq/u,
  );
...
test("DEMO cart is in-memory, deduplicates products, and never creates a server order", () => {
  ...
  assert.match(
    source,
    /current\.includes\(product\.id\) \? current : \[\.\.\.current, product\.id\]/u,
  );
  ...
  assert.match(
    source,
    /PREVIEW_PRODUCTS\.filter\(\(product\) => !cartItemIds\.includes\(product\.id\)\)\.slice\(0, 6\)/u,
  );
  assert.match(
    styles,
    /\.v2-preview-cart\.is-page \.v2-preview-cart__dock \{[^}]*position:\s*fixed;[^}]*bottom:\s*20px;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-cart\.is-page \.v2-preview-cart__dock \{[^}]*bottom:\s*calc\(74px \+ env\(safe-area-inset-bottom\)\);/u,
  );
  assert.match(source, /className="v2-preview-cart-recommendations__list"/u);
  assert.match(
    styles,
    /\.v2-preview-cart-recommendations__list\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\);/u,
  );
  assert.doesNotMatch(
    styles,
    /\.v2-preview-cart-recommendations__(?:list|rail) \{[^}]*overflow-x:\s*auto;/u,
  );
...
test("order lookup preview offers three safe DEMO-only methods", () => {
  ...
  assert.match(
    source,
    /Unknown contacts, invalid codes, and unknown order numbers share the same message/u,
  );
  assert.match(
    source,
    /"checking" \| "not-found" \| "rate-limited" \| "unavailable"/u,
  );
  assert.match(fixtures, /DEMO-ORDER-LOOKUP-02/u);
  assert.doesNotMatch(
    source,
    /v2-preview-lookup-back|返回服务目录|Back to services/u,
  );
  assert.doesNotMatch(
    source,
    /localStorage\.(?:setItem|getItem)\([^)]*(?:order|lookup)/iu,
  );
...
test("Skill preview links only to secure GitHub repositories", () => {
  ...
  const fixtures = readFileSync(fixtureFile, "utf8");
  const githubUrls = [
    ...fixtures.matchAll(/\bgithubUrl\s*:\s*["'`]([^"'`]+)["'`]/gu),
  ]
    .map((match) => match[1] ?? "")
  ...
  assert.ok(
    githubUrls.length > 0,
    "Skill fixtures must include a verified GitHub URL",
  );
  assert.equal(
    githubUrls.every((url) =>
      /^https:\/\/github\.com\/[^/\s]+\/[^/\s]+(?:\/[^\s]*)?$/u.test(url),
    ),
    true,
    `Skill links must be HTTPS GitHub URLs: ${githubUrls.join(", ")}`,
  );
  assert.ok(
    githubAnchors.length > 0,
    "Skill cards or details must render a GitHub anchor",
  );
...
test("Skill directory uses one populated taxonomy beside compact result cards", () => {
  ...
  const populatedCategories = PREVIEW_SKILL_CATEGORIES.filter(
    (category) =>
      category.key === "all" ||
      PREVIEW_SKILLS.some((skill) => skill.categoryKey === category.key),
  );
  ...
  assert.match(
    pages,
    /item\.key === "all" \|\| PREVIEW_SKILLS\.some\(\(skill\) => skill\.categoryKey === item\.key\)/u,
  );
  assert.match(pages, /className="v2-preview-skill-categories"/u);
  assert.match(
    pages,
    /className=\{`v2-preview-skills-catalog__main\$\{skillCatalogLocked/u,
  );
  assert.match(
    pages,
    /requestedCategory === "all" \|\| availableCategories\.some/u,
  );
  assert.match(pages, /replacePreviewQuery\(\{ q: query\.trim\(\), filter:/u);
  assert.doesNotMatch(
    pages,
    /v2-preview-skill-discovery[\s\S]{0,900}v2-preview-mobile-filters/u,
  );
  assert.doesNotMatch(pages, /阅读详情|Read details/u);

  assert.match(
    styles,
    /\.v2-preview-skills-catalog__main\s*\{[\s\S]{0,260}grid-template-columns:\s*clamp\(160px,\s*12vw,\s*176px\) minmax\(0,\s*1fr\);/u,
  );
  assert.match(
    styles,
    /\.v2-preview-skill-grid\s*\{[^}]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\);/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 1279px\) and \(min-width: 761px\)[\s\S]*?\.v2-preview-skill-grid\s*\{[^}]*repeat\(2,\s*minmax\(0,\s*1fr\)\)/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-skills-catalog__main\s*\{[\s\S]{0,260}grid-template-columns:\s*76px minmax\(0,\s*1fr\);/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-skill-card,[\s\S]{0,180}height:\s*116px;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 760px\)[\s\S]*?\.v2-preview-skill-discovery \{[^}]*padding:\s*8px 0 16px;/u,
  );
  assert.match(
    styles,
    /@media \(max-width: 330px\)[\s\S]*?\.v2-preview-skills-catalog__main\s*\{[^}]*grid-template-columns:\s*72px minmax\(0,\s*1fr\);/u,
  );
  assert.match(
    styles,
    /\.v2-preview-skill-card__actions a \{[^}]*min-height:\s*44px;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-skill-results \{[^}]*overflow-y:\s*auto;/u,
  );
  assert.match(
    styles,
    /\.v2-preview-skill-categories \{[^}]*height:\s*max-content;/u,
  );
});
...
test("two-level catalog fixtures keep products on secondary categories only", () => {
  const primaryKeys = new Set(
    PREVIEW_PRIMARY_CATEGORIES.map((category) => category.key),
  );
  const secondaryByKey = new Map(
    PREVIEW_SECONDARY_CATEGORIES.map((category) => [category.key, category]),
  );

  assert.deepEqual(
    PREVIEW_PRIMARY_CATEGORIES.map((category) => category.label.zh),
    ["AI 软件服务", "中转站服务"],
  );
  assert.equal(PREVIEW_SECONDARY_CATEGORIES.length, 6);
  for (const secondary of PREVIEW_SECONDARY_CATEGORIES) {
    assert.equal(
      primaryKeys.has(secondary.primaryKey),
      true,
      `${secondary.id} must reference a primary category`,
    );
  }
  for (const product of PREVIEW_PRODUCTS) {
    ...
    assert.equal(
      secondary.primaryKey,
      product.primaryCategoryKey,
      `${product.id} category ancestry must agree`,
    );
  }
});
