...
export default function SitesBackupsPage({
  ...
  const resource = useCachedAdminResource<SitesBackupSnapshot[]>("sites-backups", loader);
  ...
  const create = async () => {
    ...
    try {
      ...
      resource.commit([created, ...backups.filter((backup) => backup.id !== created.id)]);
      setReason("");
  ...
  const verify = async (id: string) => {
    ...
    try {
      ...
      resource.commit(backups.map((backup) => backup.id === id ? checked : backup));
          ...
          <>
            ...
            <div className="sites-backups-boundary" role="note">
              ...
              <span>
                ...
                {copy(
                  locale,
                  "这里验证的是可读取的加密恢复点，不会覆盖当前 D1。正式恢复必须先在隔离数据库验证表结构、记录数和管理员访问，再安排生产切换。",
                  "This verifies readable encrypted restore points and never overwrites the current D1 database. A real restore must first validate schema, counts, and administrator access in an isolated database before production cutover.",
                )}
              </span>
            </div>
            ...
            <div className="sites-backups-table-wrap" tabIndex={0}>
              <table className="sites-backups-table">
                ...
                <tbody>
                  {backups.length === 0 ? (
                    <tr><td colSpan={8}>{copy(locale, "尚无备份。打开前台后每日自动备份会开始运行，也可以立即手动创建。", "No backups yet. Daily backup starts after the storefront is opened, or create one now.")}</td></tr>
                  ) : backups.map((backup) => (
                    <tr key={backup.id}>
                      ...
                      <td>
                        <div className="sites-backups-actions">
                          <button
                            ...
                            type="button"
                          >
                            {busyId === backup.id ? copy(locale, "校验中…", "Verifying…") : copy(locale, "重新校验", "Verify")}
                          </button>
                          {backup.downloadable ? (
