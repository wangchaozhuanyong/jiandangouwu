...
export const validateSettings = (
  ...
  if (
    ...
    return {
      ...
      message: copy(
        locale,
        "中英文站点名称均为必填，且不能只包含空格。",
        "Both site names are required and cannot be blank.",
      ),
    };
  }
  if (normalized.defaultLocale !== "zh" && normalized.defaultLocale !== "en") {
    return {
      ...
      message: copy(
        locale,
        "请选择有效的默认语言。",
        "Choose a valid default language.",
      ),
    };
  }
  if (
    !normalized.seoDescription.zh ||
    ...
    return {
      ...
      message: copy(
        locale,
        "中英文 SEO 描述均为必填，且不能只包含空格。",
        "Both SEO descriptions are required and cannot be blank.",
      ),
    };
  }
  const unknownSharePlaceholder = [
    normalized.shareTemplate.zh,
    normalized.shareTemplate.en,
  ].some((template) =>
    Array.from(template.matchAll(/\{([^{}]+)\}/gu)).some(
      (match) => match[1] !== "productName" && match[1] !== "price",
    ),
  );
  if (
    !normalized.shareTemplate.zh ||
  ...
  if (!policyVersionPattern.test(normalized.policyVersion)) {
    return {
      ...
      message: copy(
        locale,
        "政策版本必须以字母或数字开头，只能包含字母、数字、点、下划线和短横线。",
        "Policy version must start with a letter or number and use only letters, numbers, dots, underscores, or hyphens.",
      ),
    };
  }
  if (normalized.transitServiceUrl) {
    try {
      ...
      return {
        ...
        message: copy(
          locale,
          "中转站地址必须是无账号密码的安全 HTTPS 地址。",
          "Transit URL must be a safe HTTPS URL without embedded credentials.",
        ),
      };
    }
  }
  if (normalized.acceptOrders && !normalized.supportEnabled) {
    return {
      ...
      message: copy(
        locale,
        "接受新订单前必须同时显示客服入口。",
        "Support access must be visible before new orders can be accepted.",
      ),
    };
  }
  if (
  ...
  if (normalized.reason.length < 8 || normalized.reason.length > 500) {
    return {
      ...
      message: copy(
        locale,
        "修改原因去除首尾空格后至少 8 个字符。",
        "Change reason must contain at least 8 characters after trimming.",
      ),
    };
  }
...
export default function SettingsPage({
  ...
  const loader = useCallback(
    (signal: AbortSignal) => getSiteSettings(signal),
    [],
  );
  const { commit, data, state, reload } =
    useCachedAdminResource<AdminStorefrontSettings>("site-settings", loader);
  ...
  const configuredActiveContactChannels =
    data?.orderReadiness?.configuredActiveContactChannels ?? 0;
  ...
  const save = async (event: React.FormEvent) => {
    ...
    const sensitiveChange =
      data.acceptOrders !== normalized.acceptOrders ||
    ...
    try {
      ...
      const message =
        ...
        requestError.code === "CONTACT_CHANNEL_REQUIRED"
          ? copy(
              locale,
              "请先在联系方式页面完成并启用至少一个真实渠道。",
              "Configure and activate at least one real contact channel first.",
          ...
          : requestError instanceof ApiError &&
              requestError.code === "ORDER_SUPPORT_REQUIRED"
            ? copy(
                locale,
                "接受新订单前必须同时显示客服入口。",
            ...
            : requestError instanceof ApiError && requestError.status === 409
              ? copy(
                  locale,
                  "网站设置已被其他管理员修改，请重新加载后再保存。",
                  "Site settings changed elsewhere. Reload before saving.",
                )
              : requestError instanceof ApiError && requestError.status === 403
                ? copy(
                    locale,
                    "当前账号没有修改网站设置的权限。",
                    "This account cannot change site settings.",
                  )
                : copy(
                    locale,
                    "网站设置未保存，请检查必填字段和 HTTPS 地址。",
                    "Site settings were not saved. Check required fields and the HTTPS URL.",
                  );
      setError(message);
    ...
    <form onSubmit={save}>
          ...
          <div>
            ...
            <button className="admin-primary" disabled={!dirty || busy}>
              ...
              {busy
                ...
                : copy(locale, "保存网站设置", "Save settings")}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="design-settings-layout real-settings-layout">
        <nav
          ...
          aria-label={copy(locale, "设置分区", "Settings sections")}
        >
          {sections.map((item) => (
            <button
        ...
        <section className="admin-panel design-settings-form">
          ...
          <fieldset className="settings-editable-fieldset" disabled={!canWrite}>
            {section === "brand" && (
              <>
                <div className="form-grid two">
                  <label>
                    <span>
                      {copy(locale, "中文站点名称", "Chinese site name")}
                    </span>
                    <input
                      value={form.siteName.zh}
                      onChange={(event) =>
                        setForm({
                          ...form,
                          siteName: {
                  ...
                  <label>
                    <span>
                      {copy(locale, "英文站点名称", "English site name")}
                    </span>
                    <input
            ...
            {section === "access" && (
              <>
                <div
                  className={`order-readiness-note${configuredActiveContactChannels > 0 ? " is-ready" : ""}`}
                  role="status"
                >
                    ...
                    <strong>
                      {configuredActiveContactChannels > 0
                        ...
                        : copy(
                            locale,
                            "接单前置条件尚未完成",
                            "Order prerequisites are incomplete",
                          )}
                    </strong>
                    <small>
                      {copy(
                      ...
                      <a href="/admin/contacts">
                        {copy(locale, "管理联系方式", "Manage contacts")}
                      </a>
                    </small>
                  </div>
                </div>
                <SettingsSwitch
                  ...
                  helpLabel={helpTriggerLabel(
                    locale,
                    copy(locale, "接受新订单", "Accept new orders"),
                  )}
                  disabled={
                    !form.acceptOrders &&
                    (configuredActiveContactChannels < 1 ||
                  ...
                  onChange={(value) =>
                    setForm({ ...form, acceptOrders: value })
                  }
                  status={
                    !form.supportEnabled
                      ? copy(
                          locale,
                          "请先显示客服入口，再开启真实接单。",
                          "Show support access before enabling live orders.",
                        )
                      : undefined
                  }
                />
                <SettingsSwitch
                  ...
                  helpLabel={helpTriggerLabel(
                    locale,
                    copy(locale, "显示客服入口", "Show support access"),
                  )}
                  disabled={
                    !form.supportEnabled && configuredActiveContactChannels < 1
                  }
                  ...
                  status={
                    configuredActiveContactChannels < 1
                      ? copy(
                          locale,
                          "至少一个真实渠道完成配置后才能显示。",
                          "Available after at least one real channel is configured.",
                        )
                      : undefined
                  }
                />
                <SettingsSwitch
                  ...
                  helpLabel={helpTriggerLabel(
                    locale,
                    copy(locale, "显示中转站服务", "Show transit service"),
                  )}
                  label={copy(locale, "显示中转站服务", "Show transit service")}
                  onChange={(value) =>
                    setForm({ ...form, transitServiceEnabled: value })
                  }
                />
                {(
                  ["HOME", "TRANSIT_SUBSCRIPTIONS", "AI_RECHARGE"] as const
                ).map((placement) => (
                  <SettingsSwitch
                    checked={form.bannerVisibility[placement]}
                    help={copy(
                      locale,
                      "只控制该展示面的广告是否公开显示，不删除广告内容。",
                      "Controls public visibility for this placement without deleting banner content.",
                    )}
                    helpLabel={helpTriggerLabel(locale, placement)}
                    key={placement}
                    label={`${copy(locale, "显示广告", "Show banners")} · ${placement}`}
                    onChange={(value) =>
                      setForm({
                        ...form,
                        bannerVisibility: {
                          ...form.bannerVisibility,
                          [placement]: value,
                        },
                      })
                    }
                ...
                <label>
                  <span className="admin-field-title">
                    {copy(
                      locale,
                      "中转站 HTTPS 地址（可留空）",
                      "Transit HTTPS URL (optional)",
                    )}
                    <HelpTip
                      label={helpTriggerLabel(
                        locale,
                        copy(locale, "中转站 HTTPS 地址", "Transit HTTPS URL"),
                      )}
                    >
                      {copy(
                        locale,
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm({
                        ...form,
                        transitServiceUrl: event.target.value || null,
                      })
                    }
                    placeholder="https://"
                    maxLength={500}
            ...
            {section === "seo" && (
              <>
                <label>
                  <span>
                    {copy(locale, "中文 SEO 描述", "Chinese SEO description")}
                  </span>
                  <textarea
                    rows={4}
                    value={form.seoDescription.zh}
                    onChange={(event) =>
                      setForm({
                        ...form,
                        seoDescription: {
                ...
                <label>
                  <span>
                    {copy(locale, "英文 SEO 描述", "English SEO description")}
                  </span>
                  <textarea
                    rows={4}
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm({ ...form, policyVersion: event.target.value })
                    }
            ...
            {section === "share" && (
              <>
                  ...
                  <div>
                    <strong>
                      {copy(
                        locale,
                        "网址由系统自动附加",
                        "The URL is appended automatically",
                      )}
                    </strong>
                    <small>
                      {copy(
                ...
                <label>
                  <span>
                    {copy(locale, "中文分享文案", "Chinese share template")}
                  </span>
                  <textarea
                    rows={4}
                ...
                <label>
                  <span>
                    {copy(locale, "英文分享文案", "English share template")}
                  </span>
                  <textarea
                    rows={4}
                ...
                <div className="settings-share-preview">
                  <small>
                    {copy(locale, "当前语言预览", "Current language preview")}
                  </small>
                  <p>
                    {form.shareTemplate[locale]
                      .replaceAll(
                        "{productName}",
                        copy(locale, "商品名称", "Product name"),
                      )
                      .replaceAll("{price}", "€19.12")}
                  </p>
                </div>
              </>
            )}
            ...
            <label className="settings-reason">
              <span>
                {copy(
                  locale,
                  "修改原因（至少 8 个字符，会写入审计）",
                  "Change reason (at least 8 characters, written to audit)",
                )}
              </span>
              <textarea
                value={form.reason}
        ...
        <aside className="admin-panel design-settings-preview">
          ...
          <dl>
            <div>
              <dt>{copy(locale, "默认语言", "Default locale")}</dt>
              <dd>{form.defaultLocale.toUpperCase()}</dd>
            </div>
            <div>
              <dt>{copy(locale, "新订单", "New orders")}</dt>
              <dd>
                {form.acceptOrders
                  ? copy(locale, "接受", "Accepted")
                  : copy(locale, "暂停", "Paused")}
              </dd>
            </div>
            <div>
              <dt>{copy(locale, "客服入口", "Support access")}</dt>
              <dd>
                {form.supportEnabled
                  ? copy(locale, "显示", "Visible")
                  : copy(locale, "隐藏", "Hidden")}
              </dd>
            </div>
            <div>
              <dt>
                {copy(locale, "库存风险阈值", "Inventory risk threshold")}
              </dt>
              <dd>1–{form.inventoryRiskThreshold}</dd>
            </div>
            <div>
              <dt>{copy(locale, "中转站", "Transit service")}</dt>
              <dd>
                {form.transitServiceEnabled
                  ? copy(locale, "首页显示", "Home only")
                  : copy(locale, "隐藏", "Hidden")}
              </dd>
            </div>
            <div>
              <dt>{copy(locale, "分享文案", "Share copy")}</dt>
              <dd>{copy(locale, "已配置", "Configured")}</dd>
            </div>
            <div>
              <dt>{copy(locale, "政策版本", "Policy version")}</dt>
              <dd>{form.policyVersion}</dd>
            </div>
          </dl>
          <div className="design-settings-warning">
            ...
            {copy(
              locale,
              "这里显示的是待保存内容；只有服务器确认后才会提示成功。",
              "This is the pending configuration. Success appears only after server confirmation.",
            )}
          </div>
        </aside>
      </div>
